...
async def start(message: Message, state: FSMContext) -> None:
    ...
    async with state.proxy() as data:
        ...
        reply: Message = await message.answer_photo(
...
async def about(message: Message) -> None:
    ...
    reply: Message = await message.answer_photo(photo=InputFile(BOT_LOGO), caption=text, disable_notification=True)
    await sleep(15)
...
async def another_city(call: CallbackQuery, state: FSMContext) -> None:
    ...
    async with state.proxy() as data:
        ...
        reply: Message = await call.bot.send_photo(
            chat_id=data['id'],
            photo=InputFile(BOT_LOGO),
            caption=await locale.get_translate(lang=data['lang'], translate='start'),
            disable_notification=True,
...
async def save_settings(call: CallbackQuery, state: FSMContext) -> None:
    ...
    async with state.proxy() as data:
        ...
        await database.save_dialog_message_id(user_id=data['id'], dialog_message_id=data['dialog_message_id'])
        text: str = await locale.get_translate(lang=data['lang'], translate='save_settings')
        reply: Message = await call.bot.send_message(chat_id=data['id'], text=f'<code>{text}</code>',
                                                     disable_notification=True)
        await sleep(15)
